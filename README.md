...
- Most child collections now have detail-view editors: vendor bids and the bid document checklist through the bespoke `/bid-evaluation` matrix, BOM parts / procurement activity schedule / post-qualification checklist through the generic line-items editor. Still without one: approval-matrix levels, purchase-order lines, milestone deliverables, and the charter funding link — flagged as the next iteration for those specifically. The UI now manages 17 top-level entities (see `src/lib/crud/configs.ts`), not the original eleven.
- The Bid Evaluation matrix locks once a Post-Qualification record for the same Procurement Activity has a decision of "Passed" — the winner's record can no longer be edited underneath the decision. This is a UI-only lock (not RLS-enforced); see the comment above `isLocked` in `BidEvaluationMatrix.tsx`.
